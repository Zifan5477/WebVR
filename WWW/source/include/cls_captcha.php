...
class Captcha{
 ...
 var $TFonts = array("captcha.ttf");
 var $TFontSize = array(15,20); //字体大小范围
 var $TFontAngle = array(-20,20); //旋转角度
 var $Chars   = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";         //验证码范围（字母数字）
 var $Code    = array();              //验证码
 var $Image   = "";              //图形对象
 ...
 public function RandRSI() ///生成验证码
 {
  ...
  $code = "";
  $font = ROOT_PATH."static/fonts/".$this->TFonts[0];
 ...
 public function Draw() ///画图
 {
  ...
  $image=$this->Image;
  $bg_color = imagecolorallocate($image, 235, 236, 237);
  imagefilledrectangle($image, 0, 0, $this->Width, $this->Height, $bg_color);
  $border_color = imagecolorallocate($image, 118, 151, 199);
